...

{
  ...
  bool haveOutgroup = false, sepPops = false, pairwiseFst = false, divergence = false, 
    lastIsOutgroup = false, windowsAnalysis = false,  slidingWindowMode = false, msflag = false, maxDisRel = false, sfsflag = false, f3stat = false;

  int slidingWindowOffset = 0, slidingWindowLength = 0, totalSampleSize = 0;

  int f3pops[3];
  vector<double> f2statvalues;
  ...
  for(int i = 1; i < argc; ++i)
	...
	f3stat = true;
	f3pops[0] = atoi(argv[++i]) -1;
	f3pops[1] = atoi(argv[++i]) -1;
	f3pops[2] = atoi(argv[++i]) -1;
	continue;
      }
  ...
  if(f3stat == true && npop < 3){
    cerr << "You cannot calculate f3 with less than 3 pops\n";
    assert(npop > 2);
  }
  ...
  //  f2statvalues = (double*)calloc( (npop * (npop -1))/2, sizeof(double));
		  ...
		  for( int pop2 = pop1 + 1; pop2 < npop; ++pop2)
		    {
		      ...
		      cout << "F2_" << pop1 << "_" << pop2 << "\t";		      
		    }
		}
	      if(f3stat == true)
		cout << "F3_" << f3pops[0] << "_" << f3pops[1] << "_" << f3pops[2] << "\t";
	    }
	}
      ...
      if(pairwiseFst == true && config != NULL)
	{
	  ...
	  for( int pop1 = 0; pop1 < npop-1; ++pop1)
	      ...
	      for( int pop2 = pop1+1; pop2 < npop; ++pop2)
		{
		  ...
		  cout << pairFST ->HBK() << "\t";
		  // cout << "pi12: " << (pairFST -> piT() - pairFST -> piS()/2 )/2. * 4. << "\t";
		  // cout << "piIN: " << pairFST -> piS() << "\t";
		  double f2 = (pairFST -> piT() - pairFST -> piS()/2 )/2. * 4. - pairFST -> piS();
		  f2statvalues.push_back(f2);
		  cout << f2 << "\t";
	  ...
	  if(f3stat == true){
	    int k1 = (npop*(npop-1)/2) - (npop-f3pops[0])*((npop-f3pops[0])-1)/2 + f3pops[1] - f3pops[0] - 1;
	    int k2 = (npop*(npop-1)/2) - (npop-f3pops[0])*((npop-f3pops[0])-1)/2 + f3pops[2] - f3pops[0] - 1;
	    int k3 = (npop*(npop-1)/2) - (npop-f3pops[1])*((npop-f3pops[1])-1)/2 + f3pops[2] - f3pops[1] - 1;
	    
	    double f3 = 0.5 * (f2statvalues[k1] + f2statvalues[k2] - f2statvalues[k3]);
	    f2statvalues.clear();
	    cout << f3 << "\t";
	  }
	}
